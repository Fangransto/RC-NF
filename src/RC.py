...
#     global X, y_train
    ...
    X = np.zeros((1 + insize + ressize, (trainlen - initlen) * trajectory_n)) #reservoir collection
    ...
    for t in range(trainlen):
        ...
        if t >= initlen:
            X[:, t - initlen::(trainlen-initlen)] = np.vstack((np.ones(trajectory_n), u, x)) #X: (ressize+isnize+1) * (len(tran-init)*trajectory_n)
    x_last = x
...
def error_cal(data, trajectory_n, last_res, outsize, insize, trainlen, errorlen, W_out, W_res, W_in, a):
    ...
    for t in range(errorlen):
        ...
        x = (1 - a) * x + a * np.tanh(np.dot(W_in, np.vstack((np.ones(trajectory_n), u))) + np.dot(W_res, x))
        y = np.dot(W_out, np.vstack((np.ones(trajectory_n), u, x)))
...
def prediction(data, trajectory_n, last_res, outsize, insize, trainlen, testlen, W_out, W_res, W_in, a):          
    ...
    for t in range(testlen):
        ...
        y = np.dot(W_out, np.vstack((np.ones(trajectory_n), u_pred, x)))
        Y[:, t] = y.reshape(-1,order = 'F')
        # generative mode:
        u_pred = y
...
#     testlen = ESN_par[4]
    ...
    # allocated memory for the design (collected states) matrix
    XX = np.zeros((1 + insize + ressize, (trainlen - initlen) * trajectory_n)) #reservoir collection
    # set the corresponding target matrix directly
    Yt = data[:, initlen + 1:trainlen + 1] # (insize*trajectory_n) * (len(tran-init))
    print('Reservoir states shape: {}\n'.format(XX.shape))
    # run the reservoir with the data and collect X
    ...
    for t in range(trainlen):
        ...
        x = (1 - a) * x + a * np.tanh(np.dot(W_in, np.vstack((np.ones(trajectory_n), u))) + np.dot(W_res, x))
        if t >= initlen:
            XX[:, t - initlen::(trainlen-initlen)] = np.vstack((np.ones(trajectory_n), u, x)) #X: (ressize+isnize+1) * (len(tran-init)*trajectory_n)
    print('**Reservoir states collection finished**')
...
def predictionError_NR(data, trajectory_n, last_res, outsize, insize, trainlen, testlen, W_out, W_res, W_in, a, errorsamples):          
    ...
    for t in range(testlen):
        x = (1 - a) * x + a * np.tanh(np.dot(W_in, np.vstack((np.ones(trajectory_n), u_pred))) + np.dot(W_res, x))
        y = np.dot(W_out, np.vstack((np.ones(trajectory_n), u_pred, x))) - error_samples[:, t*trajectory_n:(t+1)*trajectory_n]         
        # generative mode:
        u_pred = y
        Y[:, t] = y.reshape(-1,order='F')
...
#     testlen = ESN_par[4]
    ...
    for t in range(trainlen):
        if t < initlen - 1:
            ...
            u = u_train[:, t].reshape(insize, -1,order='F')
            x = (1 - a) * x + a * np.tanh(np.dot(W_in, np.vstack((np.ones(trajectory_n), u))) + np.dot(W_res, x))
            y = np.dot(W_out, np.vstack((np.ones(trajectory_n), u, x))) - errordata[:, 0].reshape(insize,-1,order='F')
            u = y
        else:                
            x = (1 - a) * x + a * np.tanh(np.dot(W_in, np.vstack((np.ones(trajectory_n), u))) + np.dot(W_res, x))
            y = np.dot(W_out, np.vstack((np.ones(trajectory_n), u, x))) - errordata[:, t-initlen].reshape(insize,-1,order='F')
