...
class temporal_NSF_AR(nn.Module):
    """
    Neural spline flow, auto-regressive.

    [Durkan et al. 2019]
    """
    def __init__(self, dim, K = 5, B = 6, hidden_dim = 32, base_network = FCNN):
        super().__init__()
        self.dim = dim
        self.K = K
        self.B = B
        ...
        for i in range(0, dim):
            self.layers += [base_network(i+1, 3 * K - 1, hidden_dim)]
        self.reset_parameters()
